...
import auth, {
  ...
  signOut,
} from '@react-native-firebase/auth'; // 🔧 CHANGED: bring in default auth + credential helpers
...
interface AuthContextType {
  ...
  // Auth methods
  // 🔧 CHANGED: return type -> Promise<void> (we drive flow with verificationId; no auto complete)
  sendOTP: (phoneNumber: string, customerType: CustomerType) => Promise<void>;
...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  // 🔧 CHANGED: stop relying on ConfirmationResult; we’ll store verificationId for manual confirm
  ...
  // -------------------------
  // OTP FLOW (Manual only)
  // -------------------------
  const sendOTP = async (phoneNumber: string, customerType: CustomerType): Promise<void> => {
    try {
      console.log('=== Starting OTP Process (manual only) ===');
      console.log('Phone number:', phoneNumber);
      console.log('Customer type:', customerType);

      // Always format to E.164
      const formattedPhone = '+91' + phoneNumber.replace(/\D/g, '');
      console.log('Formatted phone:', formattedPhone);

      setVerificationId(null);

      // 🔧 CHANGED: Use verifyPhoneNumber with state listener & ignore AUTO_VERIFIED
            ...
            (snapshot: FirebaseAuthTypes.PhoneAuthSnapshot) => {
              switch (snapshot.state) {
                ...
                case auth.PhoneAuthState.AUTO_VERIFY_TIMEOUT:
                  // We still allow manual entry after timeout
                  console.log('Auto-retrieval timed out; waiting for manual entry');
                  resolve();
                  break;

                case auth.PhoneAuthState.AUTO_VERIFIED:
                  // ❗ Do NOT sign in automatically – we’re enforcing manual entry.
                  // Optional: you could prefill snapshot.code in your UI if you want.
                  console.log('Auto-verified by Play Services; ignoring to enforce manual submit');
                  break;

                case auth.PhoneAuthState.ERROR:
                  console.log('Phone auth error (verifyPhoneNumber):', snapshot.error);
  ...
  const verifyOTP = async (otp: string, role: string): Promise<boolean> => {
    try {
      ...
      if (!verificationId) {
        throw new Error('No verificationId yet. Please request a new OTP.');
      }

      // 🔧 CHANGED: Build credential & sign in ONLY after manual submit
  ...
  const logout = async () => {
    try {
      ...
      // Reset state
      setUser(null);
      setVerificationId(null); // 🔧 CHANGED: clear manual flow state
      await clearViewAsState();
  ...
  const clearAuthData = async () => {
    await AsyncStorage.multiRemove(['accessToken', 'refreshToken', 'userProfile', 'viewAsState']);
  };
  ...
  const viewAsServiceAgent = async (
    agentId: string,
    agentName: string,
    franchiseId: string,
  ): Promise<boolean> => {
